...
export async function GET(req:Request,{params}:Ctx){
 try{
  const {id}=await params,uid=await authenticate(req),project=await requireProject(uid,id),db=admin();
  // Surface lost results even if the Studio plugin stopped polling altogether.
  const {error:expiredError}=await db.from("commands").update({
   status:"needs_reconciliation",result:{detail:"Studio response not received before deadline. Check Edit mode before resolving."}
  }).eq("project_id",id).eq("owner_id",uid).eq("status","leased").lt("lease_expires_at",new Date().toISOString());
