...
> **Early MVP deployed to Vercel, but not yet configured or live-verified end to end.** It proposes and installs basic parts, native GUI layouts (labels and visual-only button prototypes), and Luau scripts. It does not generate a complete production game, publish experiences, create 3D meshes or audio, or perform automated gameplay verification. The Asset Lab can generate private PNGs and optionally publish them as Roblox Images via per-user OAuth; a separate review step creates a visual-only Studio preview. Generated Script/LocalScript instances are disabled until you review and enable them.
...
2. The existing **Vercel project `cod-s-projects1/rsgp`** is configured as Next.js on Node 22, with a deployed site at [rsgp-five.vercel.app](https://rsgp-five.vercel.app). The public Supabase URL/key, model names, canonical origin, and encrypted Roblox token key are configured. **You still must set `SUPABASE_SERVICE_ROLE_KEY` and `OPENAI_API_KEY` as server-only Vercel variables** before application APIs work. See [.env.example](.env.example). `/api/status` reports configuration completeness without exposing secrets, and the homepage displays a warning while required credentials are missing. This is not a live dependency health test. The first deployment was made manually from the feature branch; Git automatic deployments are not configured. Do not merge the draft PR until live acceptance.
...
[The Roblox publication migration](supabase/migrations/20260922_rsgp_roblox_publication.sql) is already applied to the designated project. To enable direct upload, register an OAuth app in Roblox Creator Dashboard with the callback `https://YOUR-RSGP-ORIGIN/api/roblox/callback` and scopes `openid profile asset:read asset:write`. Configure the server-only `ROBLOX_OAUTH_CLIENT_ID` and `ROBLOX_OAUTH_CLIENT_SECRET` in Vercel; a stable `RSGP_TOKEN_ENCRYPTION_KEY` (32 random bytes, base64) was already generated and installed in Vercel. Do not rotate it without migrating stored encrypted tokens. Roblox Open Cloud OAuth and Assets API are beta; no live acceptance has been verified.
...
For local website development, use the existing Git-ignored `.env.local` in your `Desktop/Projects/RSGP` checkout. It contains the public Supabase values and stable OAuth encryption key but **not** the server-side service-role or OpenAI key. Fill those private values locally, then run `npm ci && npm run dev`. Run `npm test`, `npm run typecheck`, `npm audit --omit=dev`, and `npm run build` before deploying. The npm lockfile and patched PostCSS override are committed.
