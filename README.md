...
2. The existing **Vercel project `cod-s-projects1/rsgp`** is configured as Next.js on Node 22, with a first deployment at [rsgp-five.vercel.app](https://rsgp-five.vercel.app). The public Supabase URL/key, model names, canonical origin, and encrypted Roblox token key are configured for future deployments. **You still must set `SUPABASE_SERVICE_ROLE_KEY` and `OPENAI_API_KEY` as server-only Vercel variables** before application APIs work. See [.env.example](.env.example). The first deployment was made manually from the feature branch; Git automatic deployments are not configured. Do not merge the draft PR until live acceptance.
